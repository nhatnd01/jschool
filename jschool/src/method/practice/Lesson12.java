    ...
    public static int countString(String[] strs0, String[] strs1, String[] strs2) {
        ...
        for (int i = 0; i < strs0.length; i++) {
            if (!isDuplicated (strs0, i)) {
                if ((isContained(strs1, strs0[i]) && !isContained(strs2, strs0[i])) || (!isContained(strs1, strs0[i]) && isContained(strs2, strs0[i]))) num++;
            }
        }

        for (int i = 0; i < strs1.length; i++) {
            if (!isDuplicated (strs1, i)) {
                if (!isContained(strs0, strs1[i]) && isContained(strs2, strs1[i])) num++;
            }
        }
